...
# Separate launch for the stationary, slowly-yawing 360-degree scan scenario
# (worlds/scan_test_world.sdf.in + config/scan_test_sim.yaml). Mirrors
# landfill_camera_sim.launch.py's rendering approach but is a standalone
# file so that launch and its world/config are untouched.
...
def _setup(context):
    ...
    world_name = str(world_cfg.get('name', 'scan_test_world')).strip()
    rgb_camera_name = str(rgb_cfg.get('name', 'scan_test_rgb_camera')).strip()
    thermal_camera_name = str(thermal_cfg.get('name', 'scan_test_thermal_camera')).strip()
    lidar_name = str(lidar_cfg.get('name', 'scan_test_livox_avia_lidar')).strip()
    frame_id = str(sensors_cfg.get('frame_id', 'camera_platform/camera_link')).strip()

    xyz = _as_float_list(pose_cfg.get('xyz', [0.0, 0.0, 1.8]), 3, 'sensors.pose.xyz')
    rpy = _as_float_list(pose_cfg.get('rpy', [0.0, 0.12, 0.0]), 3, 'sensors.pose.rpy')
    ...
    gz_rgb_image_topic = _normalize_topic(
        input_topics.get('rgb_image', input_topics.get('image', '/scan_test/rgb/image_raw'))
    )
    ...
    gz_thermal_image_topic = _normalize_topic(
        input_topics.get('thermal_image', '/scan_test/thermal/image_raw')
    )
    ...
    gz_lidar_scan_topic = _normalize_topic(
        input_topics.get('lidar_scan', '/scan_test/livox/scan')
    )
    ...
    # merged_points_topic arguments should be overridden to for this
    # scenario, only used here to preselect them in the generated RViz
    # config -- see README.md for the sensor_fusion.launch.py override.
    ...
    platform_ros_cmd_topic = _normalize_topic(
        motion_cfg.get('ros_cmd_topic', '/scan_test/platform/cmd_vel')
    )
    ...
    motion_travel_distance = _non_negative_float(
        motion_cfg.get('travel_distance', 0.0),
        'platform.motion.travel_distance',
    )
    motion_linear_speed = _non_negative_float(
        motion_cfg.get('linear_speed', 0.0),
        'platform.motion.linear_speed',
    )
    motion_angular_speed = float(motion_cfg.get('angular_speed', 0.15))
    ...
    world_path = generated_dir / 'generated_scan_test_world.sdf'
    bridge_path = generated_dir / 'generated_scan_test_bridge.yaml'
    rviz_path = generated_dir / 'generated_scan_test.rviz'
    ...
    _render_template(
        package_share / 'worlds' / 'scan_test_world.sdf.in',
        world_path,
        {
...
def generate_launch_description():
    ...
    default_config = str(Path(package_share) / 'config' / 'scan_test_sim.yaml')
